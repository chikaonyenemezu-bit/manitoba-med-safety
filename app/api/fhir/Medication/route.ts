...
export async function GET(request: NextRequest) {
  ...
  if (identifier) {
    const din = identifier.includes('|') ? identifier.split('|')[1] : identifier
    query = query.eq('din', din)
  } else if (id) {
    query = query.eq('din', id)
  } else if (text) {
    query = query.or(`drug_name.ilike.%${text}%,product_name.ilike.%${text}%`)
  } else {
    return NextResponse.json(
      {
        resourceType: 'OperationOutcome',
        issue: [{ severity: 'error', code: 'required', details: { text: 'At least one search parameter is required: identifier, _text, or id' } }],
      },
  ...
  if (error) {
    return NextResponse.json(
      { resourceType: 'OperationOutcome', issue: [{ severity: 'error', code: 'exception', details: { text: error.message } }] },
